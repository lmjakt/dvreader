...
class VolumeCache {
    float* cache;
    ssize_t size;  // the size of the cache..
...
//	waveLength = wl;
    ...
    bool simpleLine(float* line, int xb, int yb, int zb, int l, Dimension dim, unsigned int wi);   // returns false if not possible
...
//	    std::cout << "VolumeCache::point point not in cache : " << xp << ", " << yp << ", " << zp << endl;
	    return(false);
	}
