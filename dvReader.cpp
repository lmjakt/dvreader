...
//    cout << "and nsec is : " << nsec << " sameEndian is : " << sameEndian << endl;
    ...
    ssize_t secSize = (pSize * nx * ny);  // the number of bytes per image..
    ssize_t imBegin = headerSize + next;  // next = n extended.. 
	...
	ssize_t framePos = imBegin + secSize * i;
	ssize_t readPos = headerSize + (nint + nreal) * 4 * i;
	ssize_t extHeadSize = next;  // I think.. 
